...
/**
 * Represents the ARO Data Collector application.  
 * The ARO Data Collector class provides methods for storing, getting, and setting data that is used during the life cycle of the application.
 * */
...
public class ARODataCollector extends Application {
	...
	public static final String ARO_TARCE_ROOTDIR ="/sdcard/ARO/";
	
	/** The error dialog ID string for the Intent filer that is used in Main Activity. 
	 *  The error dialog appears when the ARO Data Collector unexpectedly stops. */
	public static final String ERRODIALOGID = "ERRORDIALOGID";
	
	/** The name of native libs to be pushed to internal data path in sequence as per the resource id in R file*/
	private static final String mARODataCollectorNativeExe[] = {"key.db","ssldump","tcpdump"};
	
	/** The boolean value to enable logs depending on if production build or debug build*/
	...
	/**The error ID for an "SD card full" error that occurs during mid-trace or before the start of a trace. */
	...
	/**The error ID for an "SD Card mount" error that occurs before the start of a trace. */
	...
	/**The ARO-Data Collector application version String which is read from the manifest file */
	private String mApplicationVersion;
	
	/**Boolean variable to hold value true and false to check if tcpdump is active and taking trace */
	private boolean isARODataCollectorStopped;
	
	/**Boolean variable to keep true and false value for SD card mounted during mid trace */
	private boolean mMidTraceMediaMounted;
	
	/**The ARO Data Collector trace folder name */
	private String mTraceFolderName;
	
	/**The network info class object to store current connected network which is stored right after trace cycle is started*/
	private NetworkInfo mAROCurrentNetworkType;
	
	/**The notification manager class to display ARO-Data Collector alert menu notification */
	private NotificationManager mAROnotificationManager;
	
	/**String to keep trace folder name to be edited which has spaces and special characters in it */
	...
	/**Boolean value hold the value in the ARO Data Collector trace start is in progress this is first 15 sec after start collector is clicked */
	private boolean isDataCollectorStartInProgress;
	
	/** Stores true and false value to notify change in network interface bearer since time of Data Collector launch*/
	...
	/**
	 * Handles processing when an ARODataCollector object is created. Overrides the android.app.Application#onCreate method.
	...
	/**
	 * Initializes ARO Data Collector application variables, and copies the native libraries that are used by the ARO Data Collector.
	 * 
	 * @throws IOException
	 */
	public void initARODataCollector() {
		try {
			for (int resId = 0; resId < mARODataCollectorNativeExe.length; resId++) {
				PushDataCollectorExeToNative(resId,
						mARODataCollectorNativeExe[resId]);
			}
	...
	/**
	 * Initialize the Notification Manager object which ID and icon to be used to alert menu
	 */
	private void initARONotification() {
		if (mAROnotificationManager == null) {
			...
			mAROnotification = new Notification(R.drawable.icon,
					getString(R.string.aro_aleartnotification),
	...
	public void triggerAROAlertNotification() {
		...
		final CharSequence mMessage = getResources().getString(
				R.string.app_alertmenulauchtext);
		final Intent notificationIntent = new Intent(this,
				AROCollectorHomeActivity.class);
		final PendingIntent pendingIntent = PendingIntent.getActivity(this, 0,
				notificationIntent, 0);
		notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		mAROnotification.setLatestEventInfo(this, mTitle, mMessage,
				pendingIntent);
	...
	/**
	 * Copies the key.db,ssldump and tcpdump executable to native Data Collector application path
	 * 
	 * @param resourceId resource ID to be pushed in native data path
	 * @param exetuableName name of the executable 
	 * @throws IOException 
	 */
	private void PushDataCollectorExeToNative(int resourceId,
			String exetuableName) throws IOException {
		InputStream myInput = null;
		OutputStream myOutput = null;
		try {
			myInput = this.getResources().openRawResource(
					R.raw.key + resourceId);
	...
	/**
	 * Writes the specified timestamp to the trace file using the specified output file writer.
	 * 
	...
	/**
	 * Reads the tcpdump TIME_FILE value to get the start and end time of the Pcap trace.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Writes the specified time stamp that is used to calculate the video start and stop time.
	 * 
	 * @param timestamp A string value that is the timestamp.
	 * 
	 * @throws IOException 
	 */
	...
	/**
	 * Initializes the time stamp of the video file that is used to record the trace video. The time stamp Is used to calculate the start and end time of the video.
	 * 
	 * @throws FileNotFoundException 
	 */
	
	public void initVideoTraceTime() throws FileNotFoundException {
		mTraceVideoTimeStampFile = new FileOutputStream(getTcpDumpTraceFolderName() + outVideoTimeFileName);
		...
		try {
			...
			while ((ffmpegStartTimeLine = bufferReader.readLine()) != null || exitFlag) {
				...
				if (indexfound > -1) {
					...
					ffmpegstartTime = ffmpegstartTime
	...
	/**
	 * Shows the ARO Data collector progress bar for the specified UI application context.
	 * 
	 * @param appuicontext The UI application context.
	 */
	...
	public String getTcpDumpTraceFolderName() {
		return (ARO_TARCE_ROOTDIR + mTraceFolderName + "/");
	}

	/**
	 * Sets the trace folder name for the ARO Data Collector to the specified string.
	...
	/**
	 * Sets a value that enables the Video Recording collection option for an ARO Data Collector trace.
	 * 
	 * @param videoRecording A boolean value that is "true" to enable video recording for a trace, and "false" otherwise.
	 */
	 ...
	 * @return A boolean value that indicates whether or not the Video Recording collection option is enabled. 
	 */
	...
	/**
	 * Sets the network type of the currently connected network to the specified value. This value is used to determine changes in the data bearer during a trace cycle.
	 * 
	 * @param mNetworkType A Networkinfo object that indicates the network type.
	 */
	 ...
	 * 
	 * @return An int value that indicates the network type. A value of 1 indicates the current network type is “Wifi”, and a value of 0 indicates that it is any other mobile network. 
	 */
	 ...
	 * 
	 * @param tracefoldername A string that is the error trace folder name.
	 */
	 ...
	 * 
	 * @param flag A boolean value that is "true" if the tcpdump is started, and "false" otherwise.
	 */
	 ...
	 * 
	 * @param flag A boolean value that is “true” if the ARO Data Collector is in progress, and “false” if it is not. 
	 */
	 ...
	 * 
	 * @return A boolean value that indicates whether or not the ARO Data Collector is in progress.
	 */
	 ...
	 * 
	 * @return A boolean value that indicates whether or not the tcpdump has started.
	 */
	...
	/**
	 * Sets the flag that indicates whether the video capture is running during the trace.
	 * 
	 * @param flag A boolean value that is "true" if video capture is running during the trace, and "false" if it is not.
	 */
	 ...
	 * 
	 * @return A boolean value that indicates whether or not the ARO Data Collector is stopped.
	 */
	...
	/**
	 * Gets the flag that indicates whether the SD card is mounted during mid-trace.
	 * 
	 * @return A boolean value that indicates whether or not the SD card is mounted during mid-trace.
	 */
	...
	/**
	 * Sets a flag that indicates whether the SD card is mounted during mid-trace.
	 * @param flag A boolean value that is "true" if the SD card is mounted during mid-trace, and "false" if it is not.
	 */
	...
	/**
	 * Gets the flag that indicates if the video capture is running during the trace cycle.
	 * 
	 * @return A boolean value that indicates whether or not the video capture is running during the trace.
	 */
	 ...
	 * 
	 * @return A boolean value that indicates whether or not the video capture failed.
	 */
	 ...
	 * 
	 * @param flag boolean value that is "true" if the video capture has failed, and "false" if it has not.
	 */
	...
	/**
	 * Sets the flag that indicates if the ARO Data Collector data bearer has changed.
	 * 
	 * @param flag A boolean value that is "true" if the ARO Data Collector data bearer has changed, and is "false" otherwise.
	 */
	...
	/**
	 * Gets a flag that indicates if the ARO Data Collector data bearer has changed. 
	 * 
	 * @return A boolean value that indicates whether or not the ARO Data Collector data bearer changed during the trace cycle.
	 */
	...
	/**
	 * Sets a flag indicating that all listed tasks in the ARO Data Collector task killer are selected.
	 * @param flag A boolean value that is "true" if all listed tasks are selected, and "false" otherwise.
	 */
	...
	/**
	 * Gets a value that indicates if all the listed tasks in the ARO Data Collector task killer have been selected.
	 * 
	 * @return A boolean value that indicates whether or not all of the tasks in the ARO Data Collector task killer are selected.
	 */
