...
public class MinificationAnalysis {
	...
	private static final Logger LOGGER = Logger.getLogger(MinificationAnalysis.class.getName());
	...
	private void initHtmlCompressor() {
		...
		htmlCompressor.setYuiCssLineBreak(NO_LINE_BREAK);   //--line-break param for Yahoo YUI Compressor 
		htmlCompressor.setYuiJsDisableOptimizations(false); //--disable-optimizations param for Yahoo YUI Compressor 
		htmlCompressor.setYuiJsLineBreak(NO_LINE_BREAK);    //--line-break param for Yahoo YUI Compressor 
			...
			for (HttpRequestResponseInfo rr : tcpSession.getRequestResponseInfo()) {
				...
				if ((rr.getDirection() == Direction.RESPONSE) && (rr.getContentLength() != 0) && (contentType != null)) {
					...
					try {
						...
						LOGGER.log(Level.WARNING, "MinificationAnalysis - Unexpected Exception {0}", e.getMessage());
					}
				}
			}
		}
	...
	private void analyzeContent(HttpRequestResponseInfo rr, String contentType) throws Exception {
	...
	private void analyzeJavaScript(HttpRequestResponseInfo rr) throws Exception {
		...
		evaluateMinificationSavings(rr, getMinificationFileSizeSaving());
	}
	...
	private void analyzeHtml(HttpRequestResponseInfo rr) throws IOException, ContentException {
		...
		evaluateMinificationSavings(rr, getMinificationFileSizeSaving(result.originalSize, result.minifiedSize));
	}

	private void savePayloadToTmpFile(HttpRequestResponseInfo rr) throws IOException {
	...
	private void runJavaScriptMinify(boolean isJavaScript) throws IOException {
		...
		try {
			in = new InputStreamReader(new FileInputStream(tmpOriginalFile), o.charset);
			out = new OutputStreamWriter(new FileOutputStream(tmpMinifiedFile), o.charset);
			if(isJavaScript) {
			...
			LOGGER.log(Level.WARNING, "Error from JavaScript minifiaction: {0}", e.getMessage());
			throw (e);
	...
	private htmlMinificationResult runHtmlMinify(HttpRequestResponseInfo rr) throws ContentException, IOException {
		...
		try {
			...
			LOGGER.log(Level.WARNING, "Error from runHtmlMinify: {0}", e.getMessage());
			throw(e);
		} catch (IOException e) {
			LOGGER.log(Level.WARNING, "Error from runHtmlMinify: {0}", e.getMessage());
			throw(e);
	...
	private void evaluateMinificationSavings(HttpRequestResponseInfo rr, int saving) {
		if(saving > MIN_FILE_COMPRESSION) {
			this.analysisResults.add(new MinificationEntry(rr, saving));
