...
public class DuplicateResultPanel extends JPanel {
	...
	private DataTable<CacheEntry> contentTable;

	/**
	 * Initializes a new instance of the DuplicateResultPanel class. using the
	...
	private JLabel getTitle() {
		if (this.title == null) {
			this.title = new JLabel(
					Util.RB.getString("duplicate.table.header"),
	...
	public void restoreTable() {
		...
		this.scrollPane.setPreferredSize(new Dimension(
				DuplicateResultPanel.SCROLL_PANE_LENGHT,
				DuplicateResultPanel.SCROLL_PANE_HEIGHT));
	}
	 ...
	 */
	private DataTable<CacheEntry> getContentTable() {
		if (this.contentTable == null) {
			this.tableModel = new DuplicateTableModel();
			this.contentTable = new DataTable<CacheEntry>(this.tableModel);
			this.contentTable.setAutoCreateRowSorter(true);
			this.contentTable.setGridColor(Color.LIGHT_GRAY);
			this.contentTable.setRowHeight(DuplicateResultPanel.ROW_HEIGHT);
			this.contentTable.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
			this.contentTable
					.addMouseListener(getDuplicateTableMouseListener());
		}
	 ...
	 */
	private MouseListener getDuplicateTableMouseListener() {

		MouseListener ml = new MouseAdapter() {

			@Override
			public void mouseClicked(MouseEvent e) {
				CacheEntry entry = DuplicateResultPanel.this.contentTable
						.getSelectedItem();
				if ((e.getClickCount() == 2) && (entry != null)) {
					ApplicationResourceOptimizer aro = ApplicationResourceOptimizer
							.getAroFrame();
					aro.displaySimpleTab();
					aro.getAroSimpleTab().getJDuplicatesPanel()
							.getJDuplicateContentTable().selectItem(entry);
				}
			}
		};
