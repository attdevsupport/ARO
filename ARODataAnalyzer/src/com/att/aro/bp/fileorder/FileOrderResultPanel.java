...
public class FileOrderResultPanel extends JPanel implements DataTableResultPanel {
	...
	private static final Logger LOGGER = Logger
			.getLogger(FileOrderResultPanel.class.getName());
	...
	private FileOrderTableModel tableModel;
	private DataTable<FileOrderEntry> contentTable;

	/**
	 * Initializes a new instance of the FileOrderResultPanel class using the
	 ...
	 */
	public FileOrderResultPanel() {
		LOGGER.fine("init. FileOrderResultPanel");
		initialize();
	}

	/**
	 * Sets the file order result data.
	 * 
	 * @param data
	 *            - The data to be displayed in the result content table.
	 */
	public void setData(Collection<FileOrderEntry> data) {
	...
	/**
	 * Initializes the FileOrderResultPanel.
	 */
	...
	private JLabel getTitle() {
		if (title == null) {
			title = new JLabel(
					Util.RB.getString("html.fileorder.table.header"),
					JLabel.CENTER);
		}
		return title;
	}
	...
	public void restoreTable() {
		...
		this.scrollPane.setPreferredSize(new Dimension(
				FileOrderResultPanel.SCROLL_PANE_LENGHT,
				FileOrderResultPanel.SCROLL_PANE_HEIGHT));
	}
	...
	private JScrollPane getScrollPane() {
		if (scrollPane == null) {
			...
			scrollPane.setPreferredSize(new Dimension(SCROLL_PANE_LENGHT,
					SCROLL_PANE_HEIGHT));
		}
		return scrollPane;
	}
	 ...
	 */
	private DataTable<FileOrderEntry> getContentTable() {
		if (contentTable == null) {
			tableModel = new FileOrderTableModel();
			...
			contentTable.addMouseListener(getFileOrderTableMouseListener());
		}
		return contentTable;
	}

	private MouseListener getFileOrderTableMouseListener() {

		MouseListener ml;
		ml = new MouseAdapter() {

			public void mouseClicked(MouseEvent e) {
				FileOrderEntry entry = contentTable.getSelectedItem();
