...
void __stdcall SendFrameInfo(JNIEnv* env, jobject obj, ULONG ulMacType, ULONG seconds, ULONG microseconds, int ActFrameLength, BYTE* RawFrameBuf);
...
extern "C" JNIEXPORT jint JNICALL Java_com_att_aro_pcap_NetmonAdapter_parseTrace
  (JNIEnv *env, jobject obj, jstring filename){
	...
	if(l_RetValue == ERROR_SUCCESS)
	{
		...
		if(ERROR_SUCCESS == l_RetValue && l_hwnd_NplParser != INVALID_HANDLE_VALUE)
		{
			...
			if(ERROR_SUCCESS == l_RetValue && l_hwnd_ParserConfig != INVALID_HANDLE_VALUE)
			{
				//Create frame parser				
				l_RetValue = NmCreateFrameParser(l_hwnd_ParserConfig, &l_hwnd_Parser);
				if(ERROR_SUCCESS == l_RetValue && l_hwnd_Parser != INVALID_HANDLE_VALUE)
				{
						...
						for(ULONG index = 0; index < l_ulFrameCount; index++)
						{
							...
							if(l_RetValue == ERROR_SUCCESS)
							{
								...
								if(l_RetValue == ERROR_SUCCESS)
								{
									...
									SendFrameInfo(env, obj, l_ulMacType, l_ulSeconds, l_ulMicrosec, ActFrameLength, RawFrameBuf);
								}							
...
void __stdcall SendFrameInfo(JNIEnv* env, jobject obj, ULONG ulMacType, ULONG seconds, ULONG microseconds, int ActFrameLength, BYTE* RawFrameBuf)
{
	...
	mid = env->GetMethodID(cls, "pcapHandler", "(IJJI[B)V");
	if(mid != NULL) 
	{
		...
		env->CallVoidMethod(obj, mid, (jint)ulMacType, (jlong)seconds, (jlong)microseconds, (jint)ActFrameLength, dataArray);
