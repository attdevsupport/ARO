...
public class ApplicationSelection implements Serializable {
	...
	public ApplicationSelection(String appName, Collection<InetAddress> ips) {
		...
		if (ips != null) {
			for (InetAddress ipAddress : ips) {
				ipAddressSelection.put(ipAddress, new IPAddressSelection(ipAddress));
			}
		}
	}
	 ...
	 */
	public IPAddressSelection getIPAddressSelection(InetAddress ipAddress) {
		return ipAddressSelection.get(ipAddress);
	}
