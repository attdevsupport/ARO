...
public class AROCollectorSplashActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!isThisPhoneInApprovedList()) {
			//doDialogFailed();
			Log.i(TAG, "rejected");

			ArrayList<String> phoneDetails = getPhoneDetails();
			StringBuilder mssg = new StringBuilder();
			mssg.append("This device Not Supported\n\n");
			for (String line : phoneDetails) {
				mssg.append(line + "\n");
			}

			showUnapprovedPhoneError(mssg.toString());
			finish();
		} else {
			Log.i(TAG, "approved");
			doCollection();
	...
	/**
	 * If necessary, push the modified libs onto the device. If the libs were
	 * pushed, go to restart screen. Otherwise, go to the main screen.
	 */
	private void prepareSecuredCollector() {
		Intent nextActivity = null;

		// push the lib onto the device if necessary
		mApp = (ARODataCollector) getApplication();
		boolean systemLibChanged = mApp.pushSslLibsToSystemDir();
		if (systemLibChanged) {
			// go to restart screen
			nextActivity = new Intent(this, AROCollectorRestartActivity.class);
		} else {

			nextActivity = new Intent(getBaseContext(), AROCollectorMainActivity.class);
			// Generic Error ID number 100 passed as an argument to navigate to
			// Main Screen without any dialog
			nextActivity.putExtra(ARODataCollector.ERRODIALOGID, 100);
		}

		nextActivity.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		getApplication().startActivity(nextActivity);
		finish();
	}
