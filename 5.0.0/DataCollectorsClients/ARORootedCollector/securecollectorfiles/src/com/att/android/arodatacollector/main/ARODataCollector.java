...
import com.att.android.arodatacollector.utils.StreamClearer;
...
public class ARODataCollector extends Application {
	...
	private static final String ASSET_FILE_DATE_FORMAT = "yyyy-MM-dd-HH-mm";
	private static final String CHMOD_CMD_FORMAT = "chmod 666 %s\n";
	private static final String COPY_CMD_FORMAT = "cat %s > %s\n";
	private static final String SD_BACKUP_LIBS_DIR = "BACKUP_LIBS/";
	...
	/** Indicates when PIE binaries are required */
	private boolean pieMode = false;
	...
	private SimpleDateFormat sdf = new SimpleDateFormat(ASSET_FILE_DATE_FORMAT);

	/**
	 * The name of native libs to be pushed to internal data path in sequence as
	 * per the resource id in R file
	 */
	// private static final String mARODataCollectorNativeExe[] = { "key.db",
	// PROCESS_CPU_MON, "tcpdump" };
	...
	/**
	 * The error ID for an "Airplane Mode Enabled" error that occurs during
	 * mid-trace.
	 */
	 ...
	 * at https://dev.flurry.com under a specified login and Company AT&T. This
	 * is the default value meant for ATT Developers. Internal ATT users will
	 * override this value via a file input.
	 */
	public String app_flurry_api_key = "5WYSN3ZBDP476WD6VHDY";

	/**
	 * Event properties logged during ARO application for use by Flurry
	 * Analytics.
	 */
	...
	/**
	 * Event properties logged during ARO application for use by Flurry
	 * Analytics. Application context scope.
	 */
	...
	/**
	 * Indicates if the collector launch from the analyzer is in progress, (ie.
	 * waiting on the legal page or for the user to start it). This will be set
	 * to false either when the launch timeout expired, or the collector has
	 * started
	 */
	...
	public void initARODataCollector() {
		...
		try {
			int mARODataCollectorNativeExe[] = { R.raw.key, R.raw.processcpumon, R.raw.tcpdump };
			String resNames[] = { "key.db", "processcpumon.sh", "tcpdump" };
			for (int i = 0; i < mARODataCollectorNativeExe.length; i++) {
				final int resourceId = mARODataCollectorNativeExe[i];
				PushDataCollectorExeToNative(resourceId, INTERNAL_DATA_PATH + resNames[i], true);
			}
	...
	/**
	 * Required to replace the sys lib if the asset file's modified date is later than the
	 * on-device file's last modified date
	 * 
	 * @param resId
	 * @param sysLib
	 * @return
	 */
	public boolean isReplacingSysLibRequired(String assetFileName, File sysLib) {
		Date onDeviceLibLastModified = new Date(sysLib.lastModified());
		Date assetLibLastModified = getDateFromFileName(assetFileName);
		
		AROLogger.d(TAG, "onDeviceLibLastModified: " + sdf.format(onDeviceLibLastModified));
		AROLogger.d(TAG, "assetLibLastModified: " + sdf.format(assetLibLastModified));
		
		if (assetLibLastModified.after(onDeviceLibLastModified)){
			AROLogger.d(TAG, "newer asset file, need to replace existing on-device lib");
			return true;
		}
		else {
			return false;
	...
	private Date getDateFromFileName(String relativeFileName) {
		try {
			return sdf.parse(relativeFileName.substring(relativeFileName.indexOf("_") + 1));
		} catch (ParseException e) {
			AROLogger.e(TAG, "parse exception caught, using current date", e);
			return new Date();
		}
	}

	public boolean pushSslLibsToSystemDir() {
		//String[] sslLibNames = { "libcrypto.so", "libssl.so" };
		boolean sysLibChanged = false;
		String incremental = Build.VERSION.INCREMENTAL;
		String release = Build.VERSION.RELEASE;
		String codeName = Build.VERSION.CODENAME;
		String sdk = Build.VERSION.CODENAME;
		String model = Build.MODEL;
		AROLogger.d(TAG, String.format("model=%s, incremental=%s, release=%s, codeName=%s, sdk=%s", model, incremental, release, codeName, sdk));

		try {
			String[] assetLibNames = getAssets().list(release);
			for (int i = 0; i < assetLibNames.length; ++i) {
				String sslLibName = getSslLibName(assetLibNames[i]);
				final String fullDestPath = INTERNAL_DATA_PATH + sslLibName;
				File originalLib = new File(SYSTEM_LIB_DIR + sslLibName);
				
				//hierarchical name with release as parent dir. Ex: 4.0.4/libcrypto.so_2013-12-10-11-08
				
				if (isReplacingSysLibRequired(assetLibNames[i], originalLib)) {
					String assetLibPath = release + "/" + assetLibNames[i];
					sysLibChanged = true;
					// write modified libs into system dir
					pushAssetToNative(assetLibPath, fullDestPath, true);
					backupAndReplaceSysLib(sslLibName, fullDestPath);
				}

			}

		} catch (Exception e) {
			AROLogger.e(TAG, "Exception in pushSslLibsToSystemDir()", e);
		}
		
		return sysLibChanged;

	}
	
	/**
	 * The asset lib name has the format: sslLibName.so_YYYY-MM-DD-HH-mm.
	 * We're retrieving the library name part
	 * @param string
	 * @return
	 */
	private String getSslLibName(String assetLibName) {
		return assetLibName.substring(0, assetLibName.indexOf("_"));
	}


	private String getSysDirRemountCmd() {
		String commandFormat = "mount -o remount,rw -t %s %s /system\n";
		String type = "yaffs2"; // default type
		String dev = "/dev/block/mtdblock3"; // default dev

		DataOutputStream os = null;
		BufferedReader reader = null;
		try {
			Process sh = Runtime.getRuntime().exec("su"); AROLogger.e(TAG, "su pid = "+sh);
			os = new DataOutputStream(sh.getOutputStream());
			String mountInfo = "cat /proc/mounts\n";
			AROLogger.d(TAG, "issuing mount cmd");
			os.writeBytes(mountInfo);
			os.writeBytes("exit\n");

			reader = new BufferedReader(new InputStreamReader(sh.getInputStream()));
			
			String buf = null;
			AROLogger.d(TAG, "waiting to process proc/mounts output");
			...
			while ((buf = reader.readLine()) != null) {
				...
				if (buf.length() > 0 && buf.contains("/system")) {
					AROLogger.d(TAG, "system mount line: " + buf);
					
					String lineParts[] = buf.split("\\s");
					dev = lineParts[0];
					type = lineParts[2];
				}
			}
			...
			cleanupShellResources(os, reader);
		}

		return String.format(commandFormat, type, dev);
	}

	private void backupAndReplaceSysLib(String sslLibName, final String fullTempPath)
		...
		// need to mount system as read/writable every time before replacing the system file
		remountWritableSysPartition(os);
		
		changeFilePermission(sslLibName, fullTempPath, os);
		backupOriginalSysLib(os, sslLibName);
		replaceSysLib(sslLibName, fullTempPath, os);
	...
	private void replaceSysLib(String sslLibName, final String fullTempPath, DataOutputStream os)
			throws IOException {
		
		String catCommand = String.format(COPY_CMD_FORMAT, fullTempPath, SYSTEM_LIB_DIR
				+ sslLibName);
		
		AROLogger.d(TAG, "cat command: " + catCommand);
		os.writeBytes(catCommand);
	}

	private void changeFilePermission(String sslLibName, final String fullTempPath,
			DataOutputStream os) throws IOException {
		
		String sysLibChmodCmd = String.format(CHMOD_CMD_FORMAT, SYSTEM_LIB_DIR + sslLibName);
		AROLogger.d(TAG, "sysLibChmodCmd: " + sysLibChmodCmd);
		os.writeBytes(sysLibChmodCmd);

		String modLibChmodCmd = String.format(CHMOD_CMD_FORMAT, fullTempPath);
		AROLogger.d(TAG, "modLibChmodCmd: " + modLibChmodCmd);
		os.writeBytes(modLibChmodCmd);
	}

	private void remountWritableSysPartition(DataOutputStream os) throws IOException {
		/*String mountInfo = "cat /proc/mounts\n";
		AROLogger.d(TAG, "issuing mount cmd");
		os.writeBytes(mountInfo);*/

		String mountCmd = getSysDirRemountCmd();
		AROLogger.d(TAG, "mountCmd: " + mountCmd);
		os.writeBytes(mountCmd);

		/*AROLogger.d(TAG, "issuing post mount cmd");
		os.writeBytes(mountInfo);*/
	}

	private void backupOriginalSysLib(DataOutputStream os, String sslLibName) throws IOException {
		
		String sdBackupFolder = ARO_TRACE_ROOTDIR + SD_BACKUP_LIBS_DIR;
		File backupDir = new File(sdBackupFolder);
		File backupLib = new File(sdBackupFolder + sslLibName);
		
		if (backupLib.exists()){
			AROLogger.d(TAG, backupLib + " already exists; don't need to create another copy of the sys lib");
		}
		else {
			if (!backupDir.exists()){
				backupDir.mkdirs();
			...
			String backupCmd = String.format(COPY_CMD_FORMAT, SYSTEM_LIB_DIR + sslLibName, sdBackupFolder + sslLibName);
			AROLogger.d(TAG, "backupCmd: " + backupCmd);
			os.writeBytes(backupCmd);
		}
	}

	/**
	 * holds processIDs for later removal
	 */
	private HashMap<String, Process> pids = new HashMap<String, Process>();
	
	/**
	 * add a processID for later removal
	 * @param key
	 * @param pid
	 */
	 ...
	 * @param fullDestPath
	 *            full path of the destination (dir + name)
	 * @throws IOException
	 */
	private void PushDataCollectorExeToNative(int resourceId, String fullDestPath,
			boolean replaceIfExist) throws IOException {

		AROLogger.d(TAG, "pushing executable name to: " + fullDestPath);
		InputStream myInput = null;
		OutputStream myOutput = null;
		try {
			myInput = this.getResources().openRawResource(/* R.raw.key + */resourceId);
			final File file = new File(fullDestPath);
			if (file.exists()){
				if (!replaceIfExist){
					return;
				}
			...
			file.createNewFile();
			final String outFileName = fullDestPath;
	...
	private void pushAssetToNative(String relativeName, String fullDestPath,
			boolean replaceIfExist) throws IOException {

		AROLogger.d(TAG, String.format("pushing %s to %s", relativeName, fullDestPath));
		InputStream myInput = null;
		OutputStream myOutput = null;
		try {
			myInput = getAssets().open(relativeName);
			final File file = new File(fullDestPath);
	...
	public boolean getDataCollectorInProgressFlag() {
		final SharedPreferences prefs = getSharedPreferences(PREFS, 0);
		final boolean isDataCollectorStartInProgress = prefs.getBoolean(
				"isDataCollectorStartInProgress", false);
	...
	public boolean getARODataCollectorStopFlag() {
		final SharedPreferences prefs = getSharedPreferences(PREFS, 0);
		final boolean isARODataCollectorStopped = prefs.getBoolean("isARODataCollectorStopped",
				false);
		return isARODataCollectorStopped;
	}

	/**
	 * Gets the elapsedTimeStartTime which is the start time of the timer on the
	 * home page.
	 * 
	 * @return A long integer that represents when the trace started for the
	 *         home page timer.
	 */
	...
	/**
	 * Sets elapsedTimeStartTime which is the start time of the timer on the
	 * home page.
	 * 
	 * @param paramElapsedTimeStartTime
	 *            A long integer that represents when the trace started for the
	 *            home page timer.
	 */
	...
	/**
	 * Gets the flag that indicates if Airplane mode was enabled mid-trace.
	 * 
	 * @return A boolean value that indicates if Airplane mode was enabled
	 *         mid-trace.
	 */
	public boolean getAirplaneModeEnabledMidAROTrace() {
		final SharedPreferences prefs = getSharedPreferences(PREFS, 0);
		final boolean mMidTraceMediaMounted = prefs.getBoolean("mMidTraceAirplaneModeEnabled",
				false);
		return mMidTraceMediaMounted;
	}

	/**
	 * Sets a flag that indicates Airplane was enabled mid-trace.
	 * 
	 * @param flag
	...
	/**
	 * Returns a value that indicates whether or not the video.mp4 file is
	 * created in the current trace folder.
	 * 
	 * @return A boolean value that is true if the video.mp4 file exists in the
	 *         current trace folder and is larger than 0 bytes. Otherwise, the
	 *         value is false.
	 */
	public boolean isVideoFileExisting() {
		...
		if (videoFile.isFile() && videoFile.length() > 0) {
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "isVideoFileExisting(): " + "returning true"
						+ "-videoFile.isFile():" + videoFile.isFile() + "-videoFile.length():"
						+ videoFile.length());
			}
			return true;
		} else {
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "isVideoFileExisting(): " + "returning false"
						+ "-videoFile.isFile():" + videoFile.isFile() + "-videoFile.length():"
						+ videoFile.length());
			}
			return false;
	 ...
	 *            A boolean value that is true if a request to stop data
	 *            collection has been made, and is false otherwise.
	 */
	...
	/**
	 * Returns a value that indicates whether a request to stop data collection
	 * has been issued.
	 * 
	 ...
	 * 
	 * @param hashMapToWriteTo
	 *            Hashmap to write event data to.
	 * @param mapKey
	 *            Map Key to write for an event that uses a map
	 * @param mapValue
	 *            Map Value to write for an event
	 * @param eventName
	 *            Event Name to show up in Flurry Analytics
	 * @param isTimedEvent
	 *            True if event is a timed event. False otherwise
	 */
	public void writeToFlurryAndLogEvent(Map<String, String> hashMapToWriteTo, String mapKey,
			String mapValue, String eventName, boolean isTimedEvent) {
		if (hashMapToWriteTo != null) {
			...
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "logged flurry Event: " + eventName + "-hashmap key: " + mapKey
						+ "-hashmap value: " + mapValue + "-timedEvent: " + isTimedEvent);
			}
	...
	/**
	 * Writing to a Flurry hashmap if state changes but not log the event. The
	 * event should be logged at a later time.
	 * 
	 * @param hashMapToWriteTo
	 *            Hashmap to write event data to.
	 * @param mapKey
	 ...
	 *            Map Value to write for an event
	 * @param comment
	 *            Used by debug message. Set to empty string if not needed.
	 * @param existingState
	 *            used to compare states, if it is the same as last state,
	 *            hashmap is not updated. Displayed for logging.
	 * @param currentState
	 *            Current state of Event. Displayed for logging in this method.
	 */
	public void writeToFlurry(Map<String, String> hashMapToWriteTo, String mapKey, String mapValue,
			String comment, String existingState, String currentState) {
		if (hashMapToWriteTo != null) {

			hashMapToWriteTo.put(mapKey, mapValue);
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "writeToFlurry()" + "hashMapToWriteTo()-wrote flurry"
						+ "-hashmap: " + comment + "-key: " + mapKey + "-hashmap value: "
						+ mapValue + "-existingState: " + existingState);
			}
		} else {
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "writeToFlurry()" + "hashMapToWriteTo()-" + comment
						+ " not updated due to-currentState: " + currentState + "-existingState: "
						+ existingState);
			}
		}
	}
	...
	/**
	 * Need to put the receiver here so that we always receive this event and
	 * reset the analyzerLaunchWaiting variable. Putting this receiver in the
	 * activity will not always work since the broadcast wont be received if the
	 * user exits the activity and the activity gets destroyed.
	 */
	...
	private void registerAnalyzerTimeoutReceiver() {
		...
		registerReceiver(analyzerTimeoutReceiver, new IntentFilter(
				AROCollectorUtils.ANALYZER_TIMEOUT_SHUTDOWN_INTENT));
	}

	/**
	 * Returns true if PIE mode required
	 * @return pieMode; true if required
	 */
	public boolean getPieMode() {
		return pieMode;
	}

	public void cleanupShellResources(OutputStream os, Reader reader) {
		try {
			if (os != null){
				os.close();
			}
...
class FlurryEvent {
	...
	public FlurryEvent(String eventName, int eventCounter, Map<String, String> mapToWrite,
			String state) {
