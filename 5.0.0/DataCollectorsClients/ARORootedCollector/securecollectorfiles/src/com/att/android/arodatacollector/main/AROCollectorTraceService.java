...
import com.att.android.arodatacollector.utils.TLSKeyStreamer;
...
public class AROCollectorTraceService extends Service {
	...
	private static final String SSL_FILENAME = "keys.ssl";
	/**
	...
	/**
	 * Timer value to check Airplane mode enabled during trace cycle every 1
	 * second
	 */
	...
	/**
	 * Event names, counters, maps, states for displaying and storing on Flurry
	 * Analytics
	 */
	...
	private static final Pattern totalCpuLinePattern = Pattern
			.compile("^User (\\d+) \\+ Nice (\\d+) \\+ Sys (\\d+) \\+ Idle (\\d+) \\+ IOW (\\d+) \\+ IRQ (\\d+) \\+ SIRQ (\\d+) = (\\d+)");
	...
	private static String mAroTraceDatapath;
	...
	private void startSSLKeyCapture() {
		TLSKeyStreamer.startKeyCapture(mAroTraceDatapath + SSL_FILENAME);
	}
	...
	private void startAROCpuTraceScript() {
		...
		if (cpuScriptPid == -1) {
			...
			try {
				...
				command = mApp.getPieMode() ? "su -cn u:r:untrusted_app:s0 -c " : "";
	...
	private void stopAROCpuTrace() {
		/*
		 * if (cpuTimeOutTimer != null){ cpuTimeOutTimer.cancel();
		 * cpuTimeOutTimer = null; }
		 */

		/*
		 * if (scheduler != null){ cpuTraceHandle.cancel(false);
		 * scheduler.shutdown(); }
		 */
		try {
			Process sh = null;
	 ...
	 * 
	 * @param mCurrentNetworkType
	 *            network info class object to get current network type
	 */
	private void captureDeviceInfo(NetworkInfo mCurrentNetworkType) {
		...
		writeTraceLineToAROTraceFile(mDeviceDetailsWriter, getApplicationContext().getPackageName(), false);
		writeTraceLineToAROTraceFile(mDeviceDetailsWriter, deviceModel, false);

		mApp.writeToFlurry(makeModelEvent.getMapToWrite(), makeModelEvent.getEventName(), deviceMake + "/" + deviceModel, makeModelEvent.getEventName(),
	...
	// stop the ssl key capture and create the .ssl file
	public void stopSSLKeyCapture() {
		TLSKeyStreamer.stopKeyCapture();
	}
	...
	private void initAROTraceFile() throws FileNotFoundException {
		...
		mAroTraceDatapath = mApp.getTcpDumpTraceFolderName();
		if (AROLogger.logDebug) {
	...
	/**
	 * Method will close all trace file Streams and closes the files. Should set
	 * the writer to null after closing, so that a check to verify an open
	 * writer can be performed in writeTraceLineToAROTraceFile
	 * 
	 * @throws IOException
	 */
		...
		checkSDCardSpace.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				if (mAroUtils.checkSDCardMemoryAvailable() < AROSDCARD_MIN_SPACEKBYTES) {
					...
					if (AROLogger.logDebug) {
						AROLogger.d(TAG, "startARODeviceSDCardSpaceMidTrace=" + mAroUtils.checkSDCardMemoryAvailable());
					}
					return;
				}
			}
	...
	/**
	 * Notify that the Airplane Mode was enabled during trace collection and
	 * displays the error dialog by calling Main Activity
	 */
	...
	/**
	 * Checks if Airplane mode has been turned on ever every second. Only exists
	 * the trace if wifi mode is also off.
	 */
	private void startAROAirplaneModeMidTrace() {
		...
		checkAirplaneModeEnabled.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				...
				if (mAroUtils.isAirplaneModeOn(getApplicationContext()) && (wifiState == NetworkInfo.State.UNKNOWN || wifiState == NetworkInfo.State.DISCONNECTED)) {
					...
					AROLogger.d(TAG, "startAROAirplaneMidMidTrace= Airplane Mode was turned on Mid Trace");
					return;
				}
			}
	...
	private void startCameraTrace() {
		checkCameraLaunch.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				...
				if (recentTaskName.contains("camera") || checkCurrentProcessStateForGround("camera")) {
					mCameraOn = true;
				} else
					mCameraOn = false;
	...
	private BroadcastReceiver mAROScreenTraceReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (mScreenOn) {
				writeTraceLineToAROTraceFile(mScreenTracewriter, AroTraceFileConstants.ON + " " + mScreenTimeout + " " + mScreencurBrightness, true);
	...
	private void getScreenBrigthnessTimeout() {
		try {
			mScreencurBrightness = Settings.System.getInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS);
	 ...
	 * 1. bearer change between Wifi-Mobile 2. network change between 4G-3G-2G
	 * 
	...
	private void recordBearerAndNetworkChange(final NetworkInfo mAROActiveNetworkInfo, final boolean isNetworkConnected) {
		...
		if (mAROActiveNetworkInfo != null && isNetworkConnected && getDeviceNetworkType(mAROActiveNetworkInfo) != TelephonyManager.NETWORK_TYPE_UNKNOWN) {
			...
			if (!mAROPrevBearer.equals(currentBearer)) {
				...
				if (AROLogger.logDebug) {
					AROLogger.d(TAG,
							"failover, wrote networkType=" + currentNetworkType + " to networkdetails completed at timestamp: " + mAroUtils.getDataCollectorEventTimeStamp());
				}
			...
			//-1 - Wifi (We don't want to check for wifi network for 4G-3G-2G transition)
				...
				if (AROLogger.logDebug) {
					AROLogger
							.d(TAG,
									"4g-3g-2g switch, wrote networkType=" + currentNetworkType + " to networkdetails completed at timestamp: "
											+ mAroUtils.getDataCollectorEventTimeStamp());
				}
	...
	/**
	 * Records the data connection bearer change during the life time of trace
	 * collection
	 */
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(WifiManager.WIFI_STATE_CHANGED_ACTION)) {
				...
				if (mWifiManager.getWifiState() == WifiManager.WIFI_STATE_ENABLED) {
					...
					writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.DISCONNECTED_NETWORK, true);
			...
			if (WifiManager.NETWORK_STATE_CHANGED_ACTION.equals(intent.getAction())) {
				...
				case CONNECTING:
					writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.CONNECTING_NETWORK, true);
				...
				case CONNECTED:
					...
					break;
				case DISCONNECTING:
					writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.DISCONNECTING_NETWORK, true);
				...
				case DISCONNECTED:
					...
					break;
				case SUSPENDED:
					writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.SUSPENDED_NETWORK, true);
					...
					break;
				case UNKNOWN:
					writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.UNKNOWN_NETWORK, true);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
				...
				case BluetoothAdapter.STATE_ON:
					writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.DISCONNCTED, true);
					writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, getString(R.string.flurry_param_status), AroTraceFileConstants.DISCONNCTED, true);
					break;

				case BluetoothAdapter.STATE_OFF:
					writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.OFF, true);
					writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, getString(R.string.flurry_param_status), AroTraceFileConstants.OFF, true);
					break;
				}
			}
			if (BluetoothDevice.ACTION_ACL_CONNECTED.equals(action) || BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action) || BluetoothDevice.ACTION_FOUND.equals(action)) {
				...
				if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
					...
					writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.CONNECTED, true);
					writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, getString(R.string.flurry_param_status), AroTraceFileConstants.CONNECTED, true);
				}
			}
		}
	};
			...
			public void onSignalStrengthsChanged(SignalStrength signalStrength) {
				...
				if (signalStrength.isGsm() || mTelphoneManager.getNetworkType() == 13) {
					...
					if (mTelphoneManager.getNetworkType() == 13) {
						try {
							mLteSignalStrength = Integer.parseInt(mAroUtils.getSpecifiedFieldValues(SignalStrength.class, signalStrength, "mLteSignalStrength"));
						} catch (NumberFormatException nmb) {
							AROLogger.e(TAG, "mLteSignalStrength not found in LTE Signal Strength");
						}

						try {
							mLteRsrp = Integer.parseInt(mAroUtils.getSpecifiedFieldValues(SignalStrength.class, signalStrength, "mLteRsrp"));
						} catch (NumberFormatException nmb) {
							AROLogger.e(TAG, "mLteRsrp not found in LTE Signal Strength");
						}

						try {
							mLteRsrq = Integer.parseInt(mAroUtils.getSpecifiedFieldValues(SignalStrength.class, signalStrength, "mLteRsrq"));
						} catch (NumberFormatException nmb) {
							AROLogger.e(TAG, "mLteRsrq not found in LTE Signal Strength");
						}
						try {
							mLteRssnr = Integer.parseInt(mAroUtils.getSpecifiedFieldValues(SignalStrength.class, signalStrength, "mLteRssnr"));
						} catch (NumberFormatException nmb) {
							AROLogger.e(TAG, "mLteRssnr not found in LTE Signal Strength");
						}
						try {
							mLteCqi = Integer.parseInt(mAroUtils.getSpecifiedFieldValues(SignalStrength.class, signalStrength, "mLteCqi"));
						} catch (NumberFormatException nmb) {
				 ...
				 * strength values.
				 */
				else if (mTelphoneManager.getNetworkType() == TelephonyManager.NETWORK_TYPE_EVDO_0 || mTelphoneManager.getNetworkType() == TelephonyManager.NETWORK_TYPE_EVDO_A) {
	...
	private boolean isLocationServiceEnabled() {
		...
		if (gpsExists || networkExists) {
			enabled = ((!gpsExists || lm.isProviderEnabled(LocationManager.GPS_PROVIDER)) && (!networkExists || lm.isProviderEnabled(LocationManager.NETWORK_PROVIDER)));
		}
		return enabled;
	}
	...
	private void startARORadioTraceMonitor() {
		...
		mTelphoneManager.listen(mPhoneStateListener, PhoneStateListener.LISTEN_SIGNAL_STRENGTHS | PhoneStateListener.LISTEN_CALL_STATE
	...
	private void startAROScreenTraceMonitor() {
		...
		checkScreenBrightness.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				getScreenBrigthnessTimeout();
				if ((mScreencurBrightness != mPrevScreencurBrightness) || (mScreenTimeout != mPrevScreenTimeout)) {
					writeTraceLineToAROTraceFile(mScreenTracewriter, AroTraceFileConstants.ON + " " + mScreenTimeout + " " + mScreencurBrightness, true);
					mPrevScreencurBrightness = mScreencurBrightness;
					mPrevScreenTimeout = mScreenTimeout;
	...
	private void startAROBluetoothTraceMonitor() {
		...
		case BluetoothAdapter.STATE_ON:
			if (BluetoothAdapter.getDefaultAdapter().getBondedDevices().isEmpty()) {
				writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.DISCONNCTED, true);
				writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, this.getString(R.string.flurry_param_status), AroTraceFileConstants.DISCONNCTED, true);
			} else {
				writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.CONNECTED, true);
				writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, this.getString(R.string.flurry_param_status), AroTraceFileConstants.CONNECTED, true);
			}
			break;

		case BluetoothAdapter.STATE_OFF:
			writeTraceLineToAROTraceFile(mBluetoothTracewriter, AroTraceFileConstants.OFF, true);
			writeToFlurryAndMaintainStateAndLogEvent(bluetoothFlurryEvent, this.getString(R.string.flurry_param_status), AroTraceFileConstants.OFF, true);
			break;
		}
	...
	private void collectWifiNetworkData() {
		/*
		 * Get WiFi status DE9556: removed getifCurrentBearerWifi() call from if
		 * condition because when this function is called, the wifi is already
		 * connected
		 */
	...
	private void startAROActiveProcessTrace() {
		...
		for (final Iterator<RunningAppProcessInfo> iterator = mActiveProcessprocess.iterator(); iterator.hasNext();) {
			...
			case RunningAppProcessInfo.IMPORTANCE_BACKGROUND:
				...
				if (backgroundAppsFlurryEvent.getCounter() < 10) {
					mApp.writeToFlurry(backgroundAppsFlurryEvent.getMapToWrite(), runningAppProcessInfo.processName,
							AROCollectorUtils.EMPTY_STRING + backgroundAppsFlurryEvent.incrementCounter(), backgroundAppsFlurryEvent.getEventName(),
							AROCollectorUtils.NOT_APPLICABLE, AROCollectorUtils.EMPTY_STRING);
				}
			...
			case RunningAppProcessInfo.IMPORTANCE_FOREGROUND:
				mActiveProcessStates[Index] = "Name:" + runningAppProcessInfo.processName + " State:" + AroTraceFileConstants.IMPORTANCE_FOREGROUND;
				writeTraceLineToAROTraceFile(mActiveProcessTracewriter, mActiveProcessStates[Index], true);
				Index++;
				break;
			}
	...
	private void getPowerSourceStateandBatteryLevel(Intent intent) {
		...
		writeTraceLineToAROTraceFile(mBatteryTracewriter, mBatteryLevel + " " + mBatteryTemp / 10 + " " + mPowerSource, true);

		//write to Flurry only if the values change
		final String tempBatteryString = "level: " + mBatteryLevel + "%" + " " + "temp: " + mBatteryTemp / 10 + "C" + " " + "power source: " + mPowerSource;
		writeToFlurryAndMaintainStateAndLogEvent(batteryFlurryEvent, this.getString(R.string.flurry_param_status), tempBatteryString, true);
	}
	...
	private void broadcastUsbAction(boolean collectorStopEnable) {
		if (AROLogger.logDebug) {
			AROLogger.d(TAG, "broadcasting usbAction, collectorStopEnable:" + collectorStopEnable);
		}
	...
	/**
	 * method to record the screen rotation. Called when: 1. on trace start to
	 * record initial rotation 2. during trace, upon screen rotation changes
	 */
	private void recordScreenRotation() {
		...
		if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
			...
			writeTraceLineToAROTraceFile(mScreenRotationTracewriter, PORTRAIT_MODE, true);
		}
	}
	...
	public void writeToFlurryAndMaintainState(FlurryEvent fe, String key, String currentValue) {
		mApp.writeToFlurry(fe.getMapToWrite(), key, currentValue, fe.getEventName(), fe.getState(), currentValue);
	...
	/**
	 * Used by peripheral usage devices where a change in the Flurry state
	 * tracked will trigger logging of a Flurry event. Not used to log time.
	 * 
	 * @param fe
	 *            Name of a Flurry event.
	 * @param key
	 *            Map key of an event.
	 * @param currentValue
	 *            Current state value of the event. If it compares differently
	 *            than the former state, then log the event and becomes the
	 *            current state.
	 * @param timeStamp
	 *            Log time stamp as an event parameter if true. Do not log time
	 *            stamp if false.
	 */
	public void writeToFlurryAndMaintainStateAndLogEvent(FlurryEvent aFlurryEvent, String key, String currentValue, boolean timeStamp) {
		if (aFlurryEvent.getMapToWrite() != null) {
			if (timeStamp) {
				mApp.writeToFlurry(aFlurryEvent.getMapToWrite(), getString(R.string.flurry_param_time), Calendar.getInstance().getTime().toString(), aFlurryEvent.getEventName(),
						AROCollectorUtils.NOT_APPLICABLE, AROCollectorUtils.EMPTY_STRING);
			}
			mApp.writeToFlurry(aFlurryEvent.getMapToWrite(), key, currentValue, aFlurryEvent.getEventName(), aFlurryEvent.getState(), currentValue);
			if (!aFlurryEvent.getState().equals(currentValue)) {
				aFlurryEvent.setState(currentValue);
				if (AROLogger.logDebug) {
					AROLogger.d(TAG, "writeToFlurryAndMaintainStateAndLogEvent()-flurry state updated to: " + aFlurryEvent.getState() + " and logged now");
				}
			...
			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "writeToFlurryAndMaintainStateAndLogEvent()-did not log-map is null-Event: " + aFlurryEvent.getEventName() + "-key: " + key + "-value: "
						+ currentValue);
			}
		}
	}
	...
	private void recordAndLogConnectedWifiDetails() {
		...
		writeTraceLineToAROTraceFile(mWifiTracewriter, AroTraceFileConstants.CONNECTED_NETWORK + " " + mWifiMacAddress + " " + mWifiRssi + " " + mWifiNetworkSSID, true);
