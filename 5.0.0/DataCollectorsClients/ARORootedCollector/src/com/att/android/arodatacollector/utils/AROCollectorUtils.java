...
public class AROCollectorUtils {
	...
	/**
	 * used to close the old instance when a new one is launched from the
	 * analyzer
	 */
	...
	/**
	 * Retrieves the value of the specified field from an instance of the
	 * specified class using reflection.
	 * 
	 * @param mClass
	...
	public boolean checkSDCardMounted() {
		...
		if (state.equals(Environment.MEDIA_REMOVED) || !state.equals(Environment.MEDIA_MOUNTED) || state.equals(Environment.MEDIA_MOUNTED_READ_ONLY)) {
			return true;
		} else {
			return false;
		}
	...
	public String runCommand(String shellCommand) {
		...
		try {
			...
			final Thread terr = new Thread(new Runnable() {
				public void run() {
					final BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(m_process.getErrorStream()), 8192);
					String ls_1 = null;
					try {
						while ((ls_1 = bufferedReader.readLine()) != null) {
				...
				for (int itemNumPS = 0; itemNumPS < rows.length; itemNumPS++) {
					...
					if (itemNumPS == 0) {
						...
						for (int headerItemNum = 0; headerItemNum < value_item.length; headerItemNum++) {
							...
							if (AROLogger.logVerbose) {
								AROLogger.v(TAG, "header item " + headerItemNum + "=" + value_item[headerItemNum]);
							}
	...
	public String getDefaultTraceFolderName() {
		...
		final String folderName = now.get(Calendar.YEAR) + "-" + currentMonth + "-" + currentDate + "-" + currentHours + "-" + currentMinutes + "-" + currentSeconds;
	...
	/**
	 * Returns a value indicating whether tcpdump is running on the native
	 * shell.
	 * 
