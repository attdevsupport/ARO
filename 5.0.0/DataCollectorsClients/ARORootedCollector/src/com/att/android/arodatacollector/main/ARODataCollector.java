...
public class ARODataCollector extends Application {
	 ...
	 */
	private static final String mARODataCollectorNativeExe[] = { "key.db", "piemodetest", PROCESS_CPU_MON, "tcpdump", "tcpdump_pie" };
	...
	public void initARODataCollector() {
		...
		try {
			pushDataCollectorPieModeTest("piemodetest");
			testForPieMode();
		...
		try {
			for (int resId = 0; resId < mARODataCollectorNativeExe.length; resId++) {
				...
				AROLogger.d(TAG, "pull-N-push "+resId+":"+mARODataCollectorNativeExe[resId]);
	 ...
	 * @throws IOException
	 */
	private void pushDataCollectorExeToNative(int resourceId, String executableName) throws IOException {
		
		InputStream myInput = null;
		OutputStream myOutput = null;
		try {
			myInput = this.getResources().openRawResource(R.raw.key + resourceId);
			final File file = new File(INTERNAL_DATA_PATH + executableName);
			if (file.exists()) {
			...
			file.createNewFile();
			final String outFileName = INTERNAL_DATA_PATH + executableName;
	...
	/**
	 * Extract and piemodetest, a non PIE binary that fails to run on SeLinux
	 * with enforcement, but does run on no PIE environments
	 * 
	 * @param executableName
	 * @throws IOException
	 */
	private void pushDataCollectorPieModeTest(String executableName) throws IOException {

		InputStream myInput = null;
		OutputStream myOutput = null;
		try {
			myInput = this.getResources().openRawResource(R.raw.piemodetest);
			final File file = new File(INTERNAL_DATA_PATH + executableName);
			if (file.exists()) {
				file.delete();
			}
	 ...
	 */
	public boolean checkSeLinuxEnforcement() {
		...
		try {
			sh = Runtime.getRuntime().exec("getenforce");
			AROLogger.e(TAG, "getenforce = " + sh);
			os = new DataOutputStream(sh.getOutputStream());
			is = new DataInputStream(sh.getInputStream());

			int lenRead = is.read(buffer);
			if (lenRead > 0) {
				sBuffer = new String(buffer, 0, lenRead);
			}

			if (AROLogger.logDebug) {
				AROLogger.d(TAG, "getenforce=" + sBuffer);
			}

		} catch (Exception e) {
			AROLogger.e(TAG, "does not have SeLinux enforcement :"+ e.getMessage());
			pieMode = false;
		}
	 ...
	 * check for SeLinux enforcement
	 * 
	 * @return true if PIE mode is enforced
	 */
	public boolean testForPieMode() {
		...
		try {
			...
			os.writeBytes("cd " + ARODataCollector.INTERNAL_DATA_PATH + "\n");
			os.writeBytes("chmod 777 piemodetest\n");
			os.writeBytes("./piemodetest >resp.log 2>&1\n");
			os.writeBytes("cat resp.log\n");

			String responsePieMode = readShellResponce(is, buffer, "fail");

			if (responsePieMode.contains("PIE not enforced")) {
				pieMode = false;
			} else if (responsePieMode.contains("error")) {
				pieMode = true;
				os.writeBytes("setenforce 0\n");
			}

			AROLogger.i(TAG, "PIE mode flagged:" + pieMode + ", from response :<" + responsePieMode + ">");
	...
	/**
	 * Capture and return output from a Android shell's input stream.
	 * 
	 * @param is - Linux shell InputStream
	 * @param buffer - byte[] buffer to contain input stream
	 * @param defaultResponse - a default response if nothing arrived over the input stream
	 * 
	 * @return response from input stream or the default
	 */
	private String readShellResponce(DataInputStream is, byte[] buffer, String defaultResponse) {
		String response = defaultResponse;
		int lenRead = 0;
		try {
			lenRead = is.read(buffer);
		} catch (IOException e) {
			AROLogger.e(TAG, "failed to read response :"+e.getMessage());
		}
		if (lenRead > 0) {
			response = new String(buffer, 0, lenRead);
		}
		return response;
	}
	
	public static boolean isAnalyzerLaunchInProgress() {
		AROLogger.d(TAG, "isAnalyzerLaunchInProgress() returns "+isAnalyzerLaunchInProgress);
		return isAnalyzerLaunchInProgress;
	}

	public static void setAnalyzerLaunchInProgress(boolean b) {
		AROLogger.d(TAG, "setAnalyzerLaunchInProgress("+b+")");
