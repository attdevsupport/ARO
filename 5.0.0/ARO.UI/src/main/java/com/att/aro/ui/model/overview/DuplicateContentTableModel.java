 ...
 *
 */
public class DuplicateContentTableModel extends DataTableModel<CacheEntry>{
	...
	private static final String[] columns = { ResourceBundleHelper.getMessageString("duplicate.type"),
			ResourceBundleHelper.getMessageString("duplicate.time"), ResourceBundleHelper.getMessageString("duplicate.filename"),
			ResourceBundleHelper.getMessageString("duplicate.filesize") };

	/**
	 * Initializes a new instance of the CacheAnalysisTableModel class.
	 */
	public DuplicateContentTableModel() {
		super(columns);
	}
	...
	protected Object getColumnValue(CacheEntry item, int columnIndex) {
		switch (columnIndex) {
		case TYPE_COL:
			return ResourceBundleHelper.getEnumString(item.getDiagnosis());
		case TIME_COL:
